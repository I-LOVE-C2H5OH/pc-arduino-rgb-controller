...
public:
    ...
    // Добавлена инициализация ленты вместо шаблона.
    microLED(int amount, int8_t pin, int8_t pinCLK, M_chip chip, M_order order, M_ISR isr = CLI_OFF, uint8_t uptime = 0)
    :pin(pin), pinCLK(pinCLK), chip(chip), order(order), isr(isr), uptime(uptime)
    {
    }
    ...
    */
    ...
    const static int amount = 59;
    int8_t pin = 0, pinCLK = 0;
    int oneLedMax = 46;
    int oneLedIdle = 2000;
    mData leds[amount];
